...
const upstreamRepoBlobBase = "https://github.com/shakacode/shaka/blob/main";
...
// Guides surfaced in the fallback docs overview, in display order. Shaka ships its
// own docs/README.md; this overview is written only when that file is missing.
...
export function buildDocsHomeMarkdown(presentDocs, preferred = DOCS_HOME_PREFERRED) {
  ...
  return `# Shaka Documentation

Canonical source lives in [shakacode/shaka](https://github.com/shakacode/shaka/tree/main/docs).
...
async function writeDocsHome(docsRoot) {
  if (await exists(path.join(docsRoot, "README.md"))) {
    return;
  }
  const presentDocs = [];
...
export function buildChangelogMarkdown(upstreamMarkdown) {
  ...
  const frontmatter = [
    "---",
    "title: Changelog",
    'description: Release history for Shaka, synced from the upstream repository.',
    "sidebar_position: 99",
    "mdx:",
    "  format: md",
    "---",
  ...
  const intro = [
    "# Changelog",
    "",
    "Release notes for [Shaka](https://github.com/shakacode/shaka), synced from the upstream `CHANGELOG.md`.",
    ""
  ].join("\n");
